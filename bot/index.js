...
bot.on('clickButton', async (button) => {
    ...
    if(button.id === "enablebutton") {
        ...
        if(!button.message.member.hasPermission('MANAGE_SERVER')) {
            return button.message.channel.send(permsNeeded)
        }
        ...
        if(karmaGuild.enabled == true) {
            button.message.edit(new MessageEmbed().setDescription('Karma system is already **enabled** on this server.'), {buttons: [enableButton, disableButton, backButton]});
        ...
        button.message.edit(new MessageEmbed().setDescription('Karma system has been **enabled** on this server.'), {buttons: [enableButton, disableButton, backButton]});
        return button.reply.defer();
        ...
        if(karmaGuild.enabled == false) {
            button.message.edit(new MessageEmbed().setDescription('Karma system is already **disabled** on this server.'), enableButton);
            return button.reply.defer();
        }
        
        await modules.mongodb.collections.karmaSetup.findOneAndUpdate({guild_id: button.guild.id}, {$set: {enabled: false}});
        button.message.edit(new MessageEmbed().setDescription('Karma system has been **disabled** on this server.'), {buttons: [enableButton, disableButton]})
        return button.reply.defer();
        ...
        enableButton.setDisabled();
        button.message.edit(new MessageEmbed().setDescription(`Select one of the options below to continue.`).setTitle(`⭕ Karma - Setup Page`).setColor('0xFF4500'), {buttons: [emojiButton, enableButton, disableButton, resetButton]});
        return button.reply.defer();
    ...
    // emoji setup handler
        ...
        let karmaGuild = await modules.mongodb.collections.karmaSetup.findOne({guild_id: button.guild.id});
        if(karmaGuild.enabled == false) {
            return button.message.edit(new MessageEmbed().setDescription('Karma system is currently **disabled** on this server.'), {buttons: [enableButton, disableButton]});
        }
        ...
        button.message.channel.send(`Send a new emoji to replace the old one.\n*\`You have got 30 seconds!\`*`).then((msg) => {
            button.message.channel.awaitMessages(m => m.author.id === button.clicker.id, {
                ...
                if(karmaGuild.emoji1enabled == false && karmaGuild.emoji4enabled == false) {
                    return button.message.edit(new MessageEmbed().setDescription(`__Current emojis:__\n~~${karmaGuild.emoji1} \`+2 karma\`~~\n${karmaGuild.emoji2} **\`+1 karma\`**\n${karmaGuild.emoji3} **\`-1 karma\`**\n~~${karmaGuild.emoji4} \`-2 karma\`~~\n\n__How to disable 2 point emojis:__\n\`${default_prefix}karma disable plus2\` - disables +2 emoji option\n\`${default_prefix}karma disable minus2\` - disables -2 emoji option`).setColor('0xFF4500'));
        ...
        return button.message.edit(new MessageEmbed().setColor('0xFF4500').setTitle(`⭕ Karma - Disable Emoji`).setDescription(`+2 option has been enabled.`), {buttons: [enableButtonPlus, disableButtonPlus]});
        ...
        return button.message.edit(new MessageEmbed().setColor('0xFF4500').setTitle(`⭕ Karma - Disable Emoji`).setDescription(`+2 option has been disabled.`), {buttons: [enableButtonPlus, disableButtonPlus]});
        ...
        return button.message.edit(new MessageEmbed().setColor('0xFF4500').setTitle(`⭕ Karma - Disable Emoji`).setDescription(`-2 option has been enabled.`), {buttons: [enableButtonMinus, disableButtonMinus]});
        ...
        return button.message.edit(new MessageEmbed().setColor('0xFF4500').setTitle(`⭕ Karma - Disable Emoji`).setDescription(`-2 option has been disabled.`), {buttons: [enableButtonMinus, disableButtonMinus]});
...
bot.on('messageReactionAdd', async (reaction_orig, user) => {
    if (reaction_orig.message.author.id === user.id) {return;}
        ...
        if(reaction_orig._emoji.name == karmaSetup.emoji1 || karmaSetup.emoji1 == `<:${reaction_orig._emoji.name}:${reaction_orig._emoji.id}>`) {
        ...
        } else if(reaction_orig._emoji.name == karmaSetup.emoji4 || karmaSetup.emoji4 == `<:${reaction_orig._emoji.name}:${reaction_orig._emoji.id}>`) {
            var curAmount = await modules.mongodb.collections.karmaData.findOne({user_id: reaction_orig.message.author.id, guild_id: reaction_orig.message.guild.id});
            await modules.mongodb.collections.karmaData.findOneAndUpdate({user_id: reaction_orig.message.author.id, guild_id: reaction_orig.message.guild.id}, {$set: {amount: parseInt(curAmount.amount) - 2}})
